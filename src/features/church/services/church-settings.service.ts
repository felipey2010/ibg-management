...
async function requestSettings(values?: ChurchSettingsValues): Promise<ChurchSettingsResult> {
  const session = values ? await getApiSession() : null;

  if (values && (!session?.accessToken || session.user.status !== "ACTIVE")) {
    return { ok: false, status: 401, message: "Sua sessão expirou. Entre novamente." };
  }

  if (values && !hasPermission(session?.user ?? null, "church.settings.update")) {
    return {
      ok: false,
      status: 403,
